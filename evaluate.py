...
class EvalPairs(Dataset):
    """
    Dataset wrapper that returns random (cover, secret) pairs from CIFAR-10.
    """
    def __init__(self, base_ds, n=200):
        self.ds = base_ds
        self.n = n
    ...
    with torch.no_grad():
        ...
        for cover, secret in loader:
            ...
            if not first_saved:
                ...
                save_quartet_grid(
                    cover[:n], stego[:n], secret[:n], decoded[:n],
                    os.path.join(args.out_dir, 'eval_quartet_grid.png'),
                    nrow=n
                )
                first_saved = True
