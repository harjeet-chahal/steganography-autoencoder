...
def to_pil(x): # x: torch tensor in [-1,1]
...
st.image([cover_img.resize((size, size)), to_pil(stego[0])], caption=['Cover', 'Stego'])
with sc2:
st.subheader('Secret vs. Decoded')
st.image([secret_img.resize((size, size)), to_pil(decoded[0])], caption=['Secret', 'Decoded'])


st.download_button('Download Stego', data=to_pil(stego[0]).to_bytes(), file_name='stego.png', mime='image/png')
st.download_button('Download Decoded', data=to_pil(decoded[0]).to_bytes(), file_name='decoded.png', mime='image/png')
